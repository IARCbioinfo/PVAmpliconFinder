...
##	Chek if any putative known PV seq has been found
my @aecrire3;
if(%hseq_known){
	print "Putative known PV seq has been found\n";
	@aecrire3=contig_build(\%hdata_known,\%hseq_known,$inputdirfasta,$tmp3);
}
else{
	print "Not any Putative known PV seq has been found\n";
}


##########
##	NEW	##	Controles du contenu de \%hdata et/ou \%hseq (same known) value
##########
...
##	Chek if any putative novel PV seq has been found
my @aecrire;
if(%hseq){
	print "Putative new PV seq has been found\n";
	@aecrire=contig_build(\%hdata,\%hseq,$inputdirfasta,$tmp);
}
else{
	print "Not any Putative new PV seq has been found\n";
}
	...
	foreach my $pool (sort @pools){
		...
		foreach my $title (keys %$hdatafun){
			...
			if(defined($$hseqfun{$title}{$pool})){	
				...
				#~ print $tmpfun."/".$k.$k.$k.".fasta ce fichier doit exister1\n\n";
				#~ exit;
				...
				#~ print $inputdirfasta."/".$p.".fasta ce fichier doit exister2\n\n";
				#~ exit;
				...
				while (my $seq = $in->next_seq){
					...
					if($id eq ${$$hdatafun{$title}{$pool}}[10]){
						...
						#~ print $seq." avant fun\n";
						#~ print $seq->seq()." avant fun\n";
						#~ print $out."output avant fun\n";
...
sub HPVblastn{
	...
	my $id=$seqfun->display_id();
	my $sequence=$seqfun->seq();
	
	## Since January 2020 we can no more give a BioSeqIO object, but we need to give a file
	
	my $tmpfasta = $outfun =~ s/\.blast/\.fas/r;
	
	#~ print $outfun."\n";
	#~ print $tmpfasta."\n";! inc::latest is not installed
	#~ exit;
	
	open(TMP, ">".$tmpfasta) or die "$! : $tmpfasta";
	...
	open my $OUT, '>>', $outfun or die "Can't create '$outfun'";
	
	print $tmpfasta."\n";
	
	my $resultfun = $fac->blastn( -query => $tmpfasta,										#Faire le blast avec la sequence $seq -db_data => '/data/robitaillea/blastHPVdb/HPV.fasta',	
			 -outfile => $outfun, 
